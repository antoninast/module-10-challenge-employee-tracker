...
class Cli {
    ...
    viewAllEmployees() {
      ...
      JOIN employee m ON employee.manager_id = m.id;`, (err, result) => {
            if (err) {
                console.log(err);
            }
            else if (result) {
    ...
    addDepartment() {
        inquirer
            ...
            .then((res) => {
            if (res.name) {
                pool.query(`INSERT INTO department (name) VALUES ('${res.name}')`, (err, result) => {
                    if (err) {
                        console.log(err);
                    }
                    else if (result) {
    ...
    addRole() {
        ...
        setTimeout(() => {
            ...
            VALUES ('${res.title}', ${res.salary}, ${selectedDepartment[0].id})`, (err, result) => {
                        if (err) {
                            console.log(err);
                        }
                        else if (result) {
    ...
    addEmployee() {
        ...
        inquirer
            ...
            if (res.firstName && res.lastName && res.role) {
                const selectedRole = rolesData.filter((el) => el.title === res.role);
                const selectedManager = managersData.filter((el) => el.name === res.manager);
                let query = '';
    ...
    updateEmployeeRole() {
        ...
        setTimeout(() => {
            inquirer
                ...
                .then((res) => {
                if (res.employee) {
                    const selectedEmployee = employeesData.filter((el) => el.name === res.employee);
                    const selectedRole = rolesData.filter((el) => el.title === res.newRole);
                    pool.query(`
              UPDATE employee
              SET role_id = ${selectedRole[0].id}
              WHERE id = ${selectedEmployee[0].id};`, (err, result) => {
                        if (err) {
                            console.log(err);
                        }
                        else if (result) {
        ...
        setTimeout(() => {
            inquirer
                ...
                if (res.employeeId && res.managerId) {
                    const selectedEmployee = employeesData.filter((el) => el.name === res.employeeId);
                    const selectedManager = employeesData.filter((el) => el.name === res.managerId);
                    pool.query(`
              UPDATE employee
    ...
    viewEmployeesByManager() {
        ...
        GROUP BY m.id, m.first_name, m.last_name;`, (err, result) => {
            if (err) {
                console.log(err);
            }
            else if (result) {
    ...
    deleteDepartment() {
        ...
        setTimeout(() => {
            inquirer
                ...
                .then((res) => {
                if (res.department) {
                    pool.query(`DELETE FROM department WHERE department.name='${res.department}';`, (err, result) => {
                        if (err) {
                            console.log(err);
                        }
                        else if (result) {
    ...
    deleteRole() {
        ...
        setTimeout(() => {
            inquirer
                ...
                .then((res) => {
                if (res.role) {
                    pool.query(`DELETE FROM role WHERE role.title='${res.role}';`, (err, result) => {
                        if (err) {
                            console.log(err);
                        }
                        else if (result) {
        ...
        setTimeout(() => {
            inquirer
                ...
                if (res.employee) {
                    ...
                    pool.query(`DELETE FROM employee WHERE employee.id=${selectedEmployee[0].id};`, (err, result) => {
                        if (err) {
                            console.log(err);
                        }
                        else if (result) {
